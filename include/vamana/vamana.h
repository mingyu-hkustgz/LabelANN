...
namespace ANNS {
    ...
    public:
        ...
        IdxType iterate_to_fixed_point(const char *query, std::shared_ptr<SearchCache> search_cache,
            ...
            // entry point
            search_queue.insert(_entry_point,
                                _distance_handler->compute(query, _base_storage->get_vector(_entry_point), dim));
            IdxType num_cmps = 1;
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        void save(std::string &index_path_prefix) {
            std::filesystem::create_directories(index_path_prefix);
                    ...
                    for (auto j = i + 1; j < candidate_size; ++j) {
                        ...
                        occlude_factor[j] = (distance_ij == 0) ? std::numeric_limits<float>::max()
                                                               : std::max(occlude_factor[j],
                                                                          candidates[j].distance / distance_ij);
                    }
                }
                cur_alpha *= 1.2f;
            }
