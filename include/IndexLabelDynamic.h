...
#pragma omp parallel for schedule(dynamic, nthread)
    ...
    overlap_naive_search(const float *query, unsigned K, unsigned nprobs) {
        auto results = new std::pair<ANNS::IdxType, float>[query_bitmap.size() * K];
        for (int i = 0; i < query_bitmap.size(); i++) {
            ResultQueue tmp_queue;
            std::unordered_map<uint64_t, bool> mp;
            auto overbitmap = query_bitmap[i];
            while(overbitmap) {
                auto bitmap =  overbitmap & -overbitmap; // low bit
                overbitmap -= bitmap;
                if (bitmap_list[bitmap].size() >= INDEX_ELASIIC_BOUND) {
                    if(fa_[bitmap]!=bitmap){
                        auto appr_alg = appr_alg_list[fa_[bitmap]];
                        ...
                        auto hnsw_result = appr_alg->searchKnn(query + i * D, K, nprobs, &contain_filter);
                        while(!hnsw_result.empty()){
                            auto cur_res = hnsw_result.top();
                            hnsw_result.pop();
                            if(!mp[cur_res.second]){
                                tmp_queue.push(cur_res);
                                mp[cur_res.second] = true;
                            }
                        }
                    }
                    else{
                ...
                while(tmp_queue.size() > K) tmp_queue.pop();
            }
            for(int j = 0; j < K; j++){
                int tag = (int)K - j - 1;
                results[i * K + tag].first = tmp_queue.top().second;
                results[i * K + tag].second = tmp_queue.top().first;
                tmp_queue.pop();
            }
        }
        return results;
    ...
    std::pair<ANNS::IdxType, float> *
    overlap_brute_search(const float *query, unsigned K, unsigned num_threads) {
...
#else
            auto ipspace = new hnswlib::InnerProductSpace(D);
            auto appr_alg = new hnswlib::HierarchicalNSWStatic<float>(ipspace, points_num, HNSW_ELASTIC_M,
                                                                      HNSW_ELASIIC_efConstruction);
#endif
            std::cout << "\r Processing:: " << (100.0 * cumulate_points) / indexed_points << "%" << std::flush;
...
#else
        auto ipspace = new hnswlib::InnerProductSpace(D);
        appr_alg = new hnswlib::HierarchicalNSWStatic<float>(ipspace, hnsw_size, HNSW_ELASTIC_M,
                                                                      HNSW_ELASIIC_efConstruction);
#endif
