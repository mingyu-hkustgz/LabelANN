...
for data in "${datasets[@]}"; do
  ...
  ./build/tools/compute_groundtruth \
    ...
    log_file="./results/time-log/${data}/HNSW-Large-Index-time.log"
    start_time=$(date +%s)
    ./build/test/test_large_hnsw_build -d ${data} -s ./DATA/${data}/ -l ${L}_labels_zipf -e 0.2
    end_time=$(date +%s)
    duration=$((end_time - start_time))
    echo "Large[0.2] L=${L} HNSW Index time: ${duration}(s)" | tee -a ${log_file}
