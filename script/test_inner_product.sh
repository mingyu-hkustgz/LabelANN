...
#        --data_type float --dist_fn IP --scenario containment --K 10 --num_threads 144 \
...
#    start_time=$(date +%s)
#    ./build/test/test_elastic_hnsw_build_ip -d ${data} -s ./DATA/${data}/ -l ${L}_labels_${label} -e 0.2
#    end_time=$(date +%s)
#    duration=$((end_time - start_time))
#    echo "Elastic[0.2] L=${L} HNSW Index time: ${duration}(s)" | tee -a ${log_file}

    ./build/test/test_elastic_hnsw_search_ip -d ${data} -s ./DATA/${data}/ -l ${L}_labels_${label} -e 0.2

#    log_file="./results/time-log/${data}/HNSW-Elastic-Index-time.log"
#    start_time=$(date +%s)
#    ./build/test/test_elastic_hnsw_build_ip -d ${data} -s ./DATA/${data}/ -l ${L}_labels_${label} -e 2.0
#    end_time=$(date +%s)
#    duration=$((end_time - start_time))
#    echo "Elastic[2.0] L=${L} HNSW Index time: ${duration}(s)" | tee -a ${log_file}

    ./build/test/test_elastic_hnsw_search_ip -d ${data} -s ./DATA/${data}/ -l ${L}_labels_${label} -e 2.0
